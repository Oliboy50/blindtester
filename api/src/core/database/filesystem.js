...
const { DATABASE_TYPE_FILESYSTEM } = require('../../../config/const');
...
const getDatabase = async () => {
  ...
  let database;
  try {
    database = JSON.parse(await readFile(config.database[DATABASE_TYPE_FILESYSTEM].path));
    if (!database) {
...
const assignDataToDatabase = async (data) => {
  mkdirp(path.dirname(config.database[DATABASE_TYPE_FILESYSTEM].path), async (err) => {
    if (err) {
      throw err;
    }

    await writeFile(config.database[DATABASE_TYPE_FILESYSTEM].path, JSON.stringify(
